...
## KEYS — และทำไมห้ามใช้บน production
...
`KEYS *` จะ block Redis server ทั้งตัวจนกว่าจะ scan ทุก key เสร็จ บน server ที่มี key 10 ล้านตัว คำสั่งเดียวก็ block แอปพลิเคชันของคุณได้นานหลายวินาที ทำให้เกิด timeout และความล้มเหลวลูกโซ่ลามไปทุก service ที่ใช้ Redis instance นั้นร่วมกัน

`SCAN` กระจายงานออกเป็นการเรียกเล็ก ๆ หลายครั้ง แต่ละครั้งคืนค่ากลับมาอย่างรวดเร็ว แอปพลิเคชันของคุณจึงยังตอบสนองได้ ข้อแลกเปลี่ยนคือคุณอาจเห็น key ตัวหนึ่งโผล่มา แล้วถูกลบไปก่อนที่คุณจะได้ประมวลผล — ออกแบบ consumer ของคุณให้รับมือกรณีนั้นได้อย่างนุ่มนวล
...
> 💡 ตัวอย่างจากของจริง
>
> **หลักการทั่วไป** — ทีมที่ต้อง audit key ตาม pattern บน production Redis instance ที่มี key นับล้านตัว ใช้ `SCAN` แทน `KEYS` เพื่อไม่ให้กระทบ traffic จริง ทีม ops มักเขียน script ที่ loop `SCAN` เพื่อหา key ที่ตรงกับ pattern (เช่น `cache:user:*`) แล้วลบเป็น batch แทนที่จะเสี่ยง block server ด้วย `KEYS`
...
export const quizScanKeys = [
  ...
  },
  {
    ...
    options: [
      "ลบ key ที่ตรงกันออกอย่างถาวร",
      "block server ตลอดช่วงเวลาที่ scan keyspace ทั้งหมด",
      "ต้องใช้สิทธิ์ admin",
      "ทำงานได้เฉพาะกับ keyspace ที่เล็กกว่า 1000 key เท่านั้น",
    ],
    answer: 1,
  },
  {
