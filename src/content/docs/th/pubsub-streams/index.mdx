...
การตอบกลับ `(integer) 0` หมายความว่าตอนที่ publish ไม่มี subscriber เชื่อมต่ออยู่เลย เปิด session `redis-cli` ที่สอง รัน `SUBSCRIBE chat` แล้ว publish อีกครั้งเพื่อดูการส่งข้อความแบบ live
...
export const quizOverview = [
  {
    q: "subscriber ตัดการเชื่อมต่อ 5 วินาที โมเดลใดรับประกันว่าสามารถอ่านข้อความที่พลาดไปได้?",
